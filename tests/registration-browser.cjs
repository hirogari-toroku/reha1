...
(async () => {
  ...
  try {
    for (const success of [true,false]) {
      ...
      await page.goto('https://hirogari-toroku.github.io/reha1/?mode=register');
      await page.waitForFunction(() => !document.getElementById('registerOpenForm').disabled);
      if (success) await page.waitForFunction(() => document.getElementById('message').innerText.includes('保存しました'));
      await page.waitForTimeout(150);
      assert.deepEqual(calls,['logRegisterLiffLogin']);
      assert.equal(opened.length,success?1:0);
      if(success) assert.match(opened[0],/^https:\/\/docs.google.com\/forms\//);
      await page.screenshot({path:`/tmp/hirogari-registration-${success}.png`,fullPage:true});
      await page.close();
    }
